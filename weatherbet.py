...
LOCATIONS = {
    "seoul":   {"lat": 37.4691,  "lon":  126.4505, "name": "Seoul",   "station": "RKSI", "unit": "C", "region": "asia"},  # Incheon (RKSI) — Polymarket resolves on this station (Wunderground RKSI)
        ...
        for i, date in enumerate(dates):
            ...
            for market in event.get("markets", []):
                ...
                if not rng:
                    continue
                try:
                    # bestBid/bestAsk are the real YES order-book prices.
                    # outcomePrices is [P(Yes), P(No)] (sums to 1) — NOT a bid/ask quote.
                    bid = float(market.get("bestBid") or 0)
                    ask = float(market.get("bestAsk") or 0)
                    if ask <= 0:
                        # Thin market with no quote: fall back to the YES mid.
                        prices = json.loads(market.get("outcomePrices", "[0.5,0.5]"))
...
def print_markets():
    """Display all live Polymarket temperature markets with current prices and forecasts."""
    now      = datetime.now(timezone.utc)
    ...
    for city_slug, loc in LOCATIONS.items():
        ...
        print(f"  {loc['name']}  ({station} | °{unit_sym})")
        print(f"  {'-'*70}")

        # Build dates for next 4 days
        dates = [(now + timedelta(days=i)).strftime("%Y-%m-%d") for i in range(4)]
        ...
        for i, date in enumerate(dates):
            ...
            snap = snapshots.get(date, {})
            forecast_temp = snap.get("best")
            forecast_src = (snap.get("best_source") or "unknown").upper()
            forecast_str = f"{forecast_temp}°{unit_sym}  ({forecast_src})" if forecast_temp else "--   (no data)"

            print(f"  {date}  D+{i}  |  {hours:5.1f}h left  |  Forecast: {forecast_str}")
            ...
            for market in event.get("markets", []):
                ...
                try:
                    # Real YES order-book prices; outcomePrices is [P(Yes), P(No)].
                    bid = float(market.get("bestBid") or 0)
                    ask = float(market.get("bestAsk") or 0)
                    if ask <= 0:
                        prices = json.loads(market.get("outcomePrices", "[0.5,0.5]"))
            ...
            for o in outcomes:
                ...
                if t_low == -999 and t_high != 999:
                    label = f"{t_high}°{unit_sym} or below"
                elif t_low != -999 and t_high == 999:
                    label = f"{t_low}°{unit_sym} or higher"
                elif t_low == t_high:
                    label = f"{t_low}°{unit_sym}"
                else:
                    label = f"{t_low}-{t_high}°{unit_sym}"

                # Check if forecast falls in this bucket
                forecast_mark = ""
                if forecast_temp is not None and in_bucket(forecast_temp, t_low, t_high):
                    forecast_mark = "  *** FORECAST ***"

                # Format output
                bid_str = f"${o['bid']:.3f}"
                spread_str = f"${o['spread']:.3f}"
                vol_str = f"${o['volume']:,.0f}"

                print(f"    {label:<20} {bid_str:>8}   spread: {spread_str:>8}   vol: {vol_str:>10}{forecast_mark}")

            print()

        if not has_any_market:
            print("  (no markets found for any dates)")
...
if __name__ == "__main__":
    ...
    if cmd == "run":
        ...
        print("Usage: python weatherbet.py [run|status|report|markets]")
